...
import { adminAuditLogs, orders, profiles, tickets, users, } from "../db/schema.js";
 ...
 */
export async function recordAdminAudit(adminId, action, details) {
    try {
...
export async function listAdminOrders(filter) {
    ...
    if (filter.search && filter.search.trim()) {
        ...
        conditions.push(or(ilike(orders.customerName, term), ilike(orders.customerEmail, term), ilike(orders.phone, term), ilike(orders.itemName, term), ilike(orders.paymentReference, term)));
    }
 ...
 */
export async function resendOrderEmail(orderId, adminId) {
    const [order] = await db.select().from(orders).where(eq(orders.id, orderId));
    if (!order) {
        throw new Error("Order not found");
    }
...
export async function listAdminTickets(filter) {
    ...
    if (filter.search && filter.search.trim()) {
        const term = `%${filter.search.trim()}%`;
        query = query.where(or(ilike(tickets.ticketCode, term), ilike(tickets.matchName, term), ilike(orders.customerName, term), ilike(orders.customerEmail, term)));
    }
    const results = await query
 ...
 */
export async function verifyTicketCode(ticketCode, adminId) {
...
export async function listAdminUsers(filter) {
    ...
    if (filter.search && filter.search.trim()) {
        const term = `%${filter.search.trim()}%`;
        conditions.push(or(ilike(users.email, term), ilike(users.firstName, term), ilike(users.lastName, term), ilike(users.phone, term), ilike(profiles.displayName, term), ilike(profiles.city, term)));
    }
    if (conditions.length > 0) {
        query = query.where(and(...conditions));
    }
 ...
 */
export async function updateAdminUser(userId, data, adminId) {

 ...
 */
export async function sendEmailViaResend({ to, subject, html, text }: SendEmailParams) {
  const from = `${env.resend.fromName} <${env.resend.fromEmail}>`;

  if (!env.resend.apiKey) {
    console.warn(
      `[EmailService:DEV_MODE] RESEND_API_KEY is not set. Email not dispatched via network.\n` +
      `From: ${from}\nTo: ${to}\nSubject: ${subject}\n`
    );
 ...
 */
function emailLayout(content: string, preheader: string = "Bendel Insurance Football Club") {
  return `
<!DOCTYPE html>
<html lang="en">
<head>
